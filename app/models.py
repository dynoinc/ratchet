...
class Activity(Base):
    ...
    child_activities = relationship("Activity", 
...
def create_activity(db: SessionLocal, team_id: int, activity_type: ActivityType, status: ActivityStatus, content: str, timestamp: datetime = None, parent_activity_id: int = None):
    ...
    logger.info(f"Created new activity: {new_activity.id} (Type: {activity_type}, Status: {status})")
    return new_activity
...
def update_activity(db: SessionLocal, activity_id: int, new_status: ActivityStatus = None, new_content: str = None):
...
def get_or_create_channel_status(db: SessionLocal, slack_channel_id: str):
    channel_status = db.query(ChannelProcessingStatus).filter_by(slack_channel_id=slack_channel_id).first()
...
def update_channel_status(db: SessionLocal, slack_channel_id: str, new_timestamp: float):
    channel_status = get_or_create_channel_status(db, slack_channel_id)
    channel_status.last_processed_timestamp = new_timestamp
    db.commit()
    logger.info(f"Updated channel status for {slack_channel_id}: new timestamp {new_timestamp}")
    return channel_status
